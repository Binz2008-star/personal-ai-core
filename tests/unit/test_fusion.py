    ...
    return CandidateList(
        ...
        candidates=tuple(
            Candidate(
                chunk_id=chunk_id,
                rank=rank,
                # Deliberately on a wildly different scale from the semantic
...
def test_score_is_the_sum_of_one_over_k_plus_rank(fusion):
    fused = fusion.fuse_detailed([semantic("a"), lexical("a")], limit=5)
...
def test_a_candidate_missing_from_a_list_contributes_zero_not_a_penalty(fusion):
    ...
    fused = {f.chunk_id: f.fused_score for f in fusion.fuse_detailed(
        [semantic("a", "b"), lexical("a")], limit=5
    )}
...
def test_fusion_reads_ranks_and_ignores_scores(fusion):
    ...
    exists at all.
    """
    order = fusion.fuse([semantic("a", "b"), lexical("b", "a")], limit=5)
...
def test_appearing_in_both_arms_beats_appearing_first_in_one(fusion):
    order = fusion.fuse([semantic("only-top", "both"), lexical("both")], limit=5)
...
def test_equal_scores_order_by_ascending_chunk_id(fusion):
    order = fusion.fuse([semantic("z"), semantic("a"), semantic("m")], limit=5)
    assert order == ["a", "m", "z"]
...
def test_ordering_does_not_depend_on_input_order(fusion):
    ...
    first = fusion.fuse([semantic("b", "a"), lexical("a", "b")], limit=5)
    second = fusion.fuse([lexical("a", "b"), semantic("b", "a")], limit=5)
    assert first == second
...
def test_contributions_record_what_each_arm_supplied(fusion):
    fused = fusion.fuse_detailed([semantic("a", "b"), lexical("b")], limit=5)
    by_id = {f.chunk_id: f for f in fused}
    assert by_id["b"].methods() == (RetrievalMethod.SEMANTIC, RetrievalMethod.LEXICAL)
    assert by_id["a"].methods() == (RetrievalMethod.SEMANTIC,)
    assert {c.rank for c in by_id["b"].contributions} == {2, 1}


def test_fuse_returns_the_same_order_as_fuse_detailed(fusion):
    lists = [semantic("a", "b", "c"), lexical("c", "a")]
...
def test_limit_truncates_after_ranking_not_before(fusion):
    ...
    assert fusion.fuse([semantic("a", "b"), lexical("b", "b2")], limit=1) == ["b"]
...
def test_empty_lists_produce_no_results(fusion):
    assert fusion.fuse([CandidateList(method=RetrievalMethod.SEMANTIC)], limit=5) == []


@pytest.mark.parametrize("limit", [0, -1])
def test_a_non_positive_limit_is_rejected(fusion, limit):
    with pytest.raises(ValueError):
        fusion.fuse([semantic("a")], limit=limit)
